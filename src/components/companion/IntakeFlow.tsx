...
const SAMPLE_PASTE = `Day 1 - Arrive Berlin, Brandenburg Gate & Kreuzberg dinner
Day 2 - Museum Island, East Side Gallery, Alexanderplatz
Day 3 - Train to Munich, Marienplatz, beer garden evening
Day 4 - Neuschwanstein Castle day trip, Bavarian Alps views
Day 5 - Train to Heidelberg, castle ruins, Old Bridge sunset
Day 6 - Rhine Valley cruise, Cologne Cathedral, local Kölsch pub
Day 7 - Cologne old town, chocolate museum, farewell dinner`;
...
export default function IntakeFlow() {
    ...
    <>
      ...
      <div style={{
        ...
        <div style={{ fontSize: 14, opacity: 0.85, lineHeight: 1.5, maxWidth: 300 }}>
          Turn any Germany itinerary into an offline trip companion with local directions, budget tracking, and insider tips.
        </div>
      </div>
      ...
      <PathCard
        ...
        desc="Popular routes you can customize — Bavaria, Rhine, Berlin & more"
      ...
      <div style={{
        ...
        <div style={{ fontSize: 13, color: COLORS.deepNight, lineHeight: 1.6, fontStyle: 'italic' }}>
          "We had zero signal on the train through the Black Forest and the app had everything — directions, restaurant picks, even the right phrases. Felt like traveling with a local friend."
        </div>
        <div style={{ fontSize: 12, fontWeight: 700, color: COLORS.oceanTeal, marginTop: 8 }}>
          — Sarah & Mike, 7-day Bavaria trip
        </div>
      </div>
    </>
  );
  ...
  const TemplatesScreen = () => {
    ...
    const filtered = filter === 'all'
      ? templates
      : templates.filter((t) => {
          if (filter === 'bavaria') return t.route.match(/Munich|Nuremberg|Rothenburg/i);
          if (filter === 'rhineland') return t.route.match(/Cologne|Rhine Valley/i);
          return true;
        });
    ...
    <>
      ...
      {!apiError && (
        <>
          ...
          {[
            { icon: '🧭', title: 'Real Directions', desc: "Not Google Maps pins — actual 'take the S-Bahn to Marienplatz, exit south' directions" },
            { icon: '🌅', title: 'Sunrise/Sunset', desc: 'Golden hour times with best viewpoints matched to your daily locations' },
            { icon: '💬', title: 'Local Phrases', desc: 'German phrases tuned to your destinations with pronunciation' },
            ...
            { icon: '💵', title: 'Tipping Guide', desc: 'What to tip in every Germany situation, from restaurants to taxi drivers' },
            { icon: '🌅', title: 'Sunrise/Sunset', desc: 'Golden hour times with best viewpoints matched to your daily locations' },
            { icon: '📶', title: 'Works Offline', desc: 'Everything cached to your phone. No signal on the train? No problem' },
          ].map((f, i) => (
            <div key={i} style={{
