...
interface ApiDayItem {
  ...
  priceEur?: number;
  priceUsd?: number;
  category: 'transport' | 'accommodation' | 'activity' | 'food' | 'ferry' | 'train';
...
function mapCategory(category: ApiDayItem['category']): Activity['type'] {
  if (category === 'ferry' || category === 'train') return 'transport';
  return category;
}
...
function splitDescription(description: string, priceEur?: number, priceUsd?: number): { title: string; detail: string } {
  ...
  if (priceEur && priceUsd) {
    detail += ` (€${priceEur.toLocaleString()} / $${priceUsd})`;
  }
...
export function transformItinerary(apiItinerary: ApiItinerary): TripData {
  ...
  const days: TripDay[] = apiItinerary.days.map((apiDay) => ({
    ...
    items: apiDay.items.map((item, idx): Activity => {
      const { title, detail } = splitDescription(item.description, item.priceEur, item.priceUsd);
      return {
  ...
  const contacts: Contact[] = [
    { icon: '🚨', name: 'German Emergency', role: 'Emergency Services', phone: '911' },
    { icon: '🏥', name: 'German Red Cross', role: 'Nationwide', phone: '143' },
  ];
