...
interface DayItem {
  ...
  priceEur?: number;
  priceUsd?: number;
  category: 'transport' | 'accommodation' | 'activity' | 'food' | 'train';
...
interface Itinerary {
  title: string;
  subtitle: string;
  totalBudget: { eur: number; usd: number };
  days: Day[];
}
...
function extractDestsFromText(text: string): string[] {
  ...
  const themeMap: Record<string, string[]> = {
    'beach': ['hamburg', 'cologne', 'black-forest'],
    'castle': ['heidelberg', 'rothenburg', 'nuremberg'],
    'art': ['berlin', 'dresden', 'munich'],
    'culture': ['berlin', 'munich', 'hamburg'],
    'history': ['berlin', 'nuremberg', 'dresden'],
    'food': ['munich', 'cologne', 'hamburg'],
    'wine': ['rhine-valley', 'heidelberg', 'stuttgart'],
    'relax': ['black-forest', 'rhine-valley', 'heidelberg'],
    'adventure': ['black-forest', 'munich', 'hamburg'],
    'family': ['berlin', 'munich', 'hamburg'],
    'honeymoon': ['heidelberg', 'rothenburg', 'rhine-valley'],
    'romantic': ['heidelberg', 'rothenburg', 'dresden'],
    'budget': ['berlin', 'leipzig', 'hamburg'],
    'luxury': ['munich', 'hamburg', 'heidelberg'],
    'bavaria': ['munich', 'nuremberg', 'rothenburg'],
    'rhineland': ['cologne', 'rhine-valley', 'heidelberg'],
    'northern': ['hamburg', 'berlin', 'leipzig'],
    'eastern': ['berlin', 'dresden', 'leipzig'],
    'hiking': ['black-forest', 'munich', 'heidelberg'],
    'beer': ['munich', 'cologne', 'nuremberg'],
    'christmas': ['nuremberg', 'cologne', 'rothenburg'],
  };
...
function buildSystemPrompt(): string {
  return `You are the Discovering Germany AI Trip Planner. You generate detailed day-by-day itineraries for Germany travel.

RULES:
- All prices in BOTH EUR and USD ($). Use rate: 1 EUR = $1.09 USD
...
- Be specific: real place names, real prices, real transport options (Deutsche Bahn ICE trains, FlixBus, Lufthansa, Eurowings)
- Tag hotel/tour/transport items with affiliateType and affiliateSlotId for future monetization
- affiliateSlotId format: "day{N}-{type}-{destination}" e.g. "day1-hotel-berlin"
- Transport between destinations should reference real train routes (e.g. Berlin to Munich via ICE), domestic flights via Lufthansa or Eurowings, or FlixBus services
...
- 15+ days: 2-3 items per day, brief descriptions (under 15 words each). Group similar days (e.g. "Days 5-6: Exploring German island villages"). Only include key activities, one meal, and accommodation.
...
{
  "title": "string — trip title like '7-Day Northern Germany Grand Tour'",
  "subtitle": "string — budget + month like 'Mid-Range · March 2026'",
  "totalBudget": { "eur": number, "usd": number },
  "days": [
    {
      "dayNumber": 1,
      "title": "string — day title like 'Arrive in Berlin'",
      "destination": "string — slug like 'berlin'",
      "items": [
        {
          "time": "string — like 'Morning' or '9:00 AM'",
          "description": "string — what to do, with personality",
          "priceEur": number_or_null,
          "priceUsd": number_or_null,
          "category": "transport|accommodation|activity|food|train",
          "affiliateType": "hotel|tour|transport|null",
          "affiliateSlotId": "string_or_null",
          "locationName": "string — specific place name for map pin, e.g. 'Brandenburg Gate', 'Marienplatz', 'Heidelberg Castle'. Use real, specific place names."
        }
      ]
    }
  ]
...
function buildUserPrompt(body: RequestBody, destinations: string, duration: string, budgetLevel: string): string {
  ...
  let prompt = `Plan a ${duration}-day Germany trip:\n${parts.join('\n')}`;
  const days = parseInt(duration);
  if (days >= 15) {
    prompt += `\n\nIMPORTANT: This is a long trip. Keep descriptions under 15 words. Use 2-3 items per day max. Group similar days together. Stay well under 7000 tokens.`;
  } else if (days >= 8) {
